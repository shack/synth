from eval.util import SynthRun, ComparisonExperiment, get_solvers
            ...
            b: {
                s: [
                    SynthRun(set=set, bench=b, synth=s, iteration=i,
                             timeout=timeout, run_opts=run_difficult)
...
class Downscale(ComparisonExperiment):
    def __init__(self, iterations, timeout=10*60, set=hackdel_light):
        ...
        self.exp = {
            b: {
                w: [
                    SynthRun(set=set, bench=b, synth='downscale', iteration=i, timeout=timeout,
        ...
        self.exp = {
            b: {
                s: [
                    ...
                    for i in range(iterations)
                ] for s, path in get_solvers().items()
            } for b in benches
        }

def experiments(n_runs):
    return [
        SynthComparison(n_runs),
        Downscale(n_runs),
        Solvers(n_runs),
    ]
...
class SyGuS(ComparisonExperiment):
    def __init__(self, difficulty: int, iterations=3, timeout=10*60):
