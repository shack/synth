...
constraint = Constraint(
    phi=Exists([r], And(sum, smaller_sum)),
    params=[x],
...
func = SynthFunc(
    ...
    ops={ op: None for op in Bv(smaller_width).simple_ops }
)
...
prgs, stats = LenCegis(debug=Debug(what="len|cex|synth_prg|verif_constr|verif_model"), verbose=True).synth_prgs(problem)
if prgs:
